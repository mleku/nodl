...
func GetDataDir(goos, appName string, roaming bool) string {
	...
	// The caller really shouldn't prepend the appName with a period, but if
	// they do, handle it gracefully by trimming it.
	...
	// Windows.
	case "windows":
		// Windows XP and before didn't have a LOCALAPPDATA, so fallback to
		// regular APPDATA when LOCALAPPDATA is not set.
