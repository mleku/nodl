...
func help(rl *R, prefix string, ev *event.T, cmd *Command, args ...string) (reply *event.T, err error) {
...
func set(rl *R, prefix string, ev *event.T, cmd *Command, args ...string) (reply *event.T, err error) {
	var replyString string
	if len(args) < 3 {
...
func list(rl *R, prefix string, ev *event.T, cmd *Command, args ...string) (reply *event.T, err error) {
	replyString := fmt.Sprintf("list not implemented yet\n args: %v\nevent: %s\nprefix: %s",
		args, ev.String(), prefix)
	log.I.F("sending message to user\n%s", replyString)
	reply = MakeReply(ev, replyString)
	return
}
