...
type BaseConfig struct {
	...
	// AllowIPs is for bypassing authentication required for clients based on IP addresses... primarily for testing with
	// wireguard VPN clients run by the developer, as these are stable, non-routeable addresses, this skips the
	...
	// DBSizeLimit configures a target maximum size to maintain the local event store cache at, in megabytes (1,000,000
	// bytes).
	DBSizeLimit int `arg:"-S,--sizelimit" json:"db_size_limit" help:"set the maximum size of the badger event store in megabytes"` // default:"0"
	// DBLowWater is the proportion of the DBSizeLimit to prune the database down to when performing a garbage
	// collection run.
	...
	// DBHighWater is the proportion of the DBSizeLimit at which a garbage collection run is triggered.
	...
	// GCFrequency is the frequency to run a check on the database size and if it breaches DBHighWater to prune it back
	// to DBLowWater percentage of DBSizeLimit in minutes.
...
// these must be implemented specifically for variants, basically can be copy/pasted
// func (c *BaseConfig) Save(filename string) (err error) {
// 	if c == nil {
...
// }
//
// func (c *BaseConfig) Load(filename string) (err error) {
// 	if c == nil {
